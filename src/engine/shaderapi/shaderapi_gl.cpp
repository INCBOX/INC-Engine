...
bool ShaderAPI_GL::ShaderProgram::Compile(const char* vertexSrc, const char* fragmentSrc) {
...
void ShaderAPI_GL::ShaderProgram::Use() const {
    glUseProgram(ID);
}

void ShaderAPI_GL::ShaderProgram::Delete() {
    if (ID) {
        glDeleteProgram(ID);
        ID = 0;
    }
...
bool ShaderAPI_GL::ShaderProgram::CheckCompileErrors(unsigned int shader, const char* type) {
    int success;
    char infoLog[512];
...
bool ShaderAPI_GL::ShaderProgram::CheckLinkErrors(unsigned int program) {
    int success;
    char infoLog[512];
...
ShaderAPI_GL::VertexBuffer::VertexBuffer() {
    glGenBuffers(1, &ID);
}
...
void ShaderAPI_GL::VertexBuffer::Bind() const {
    glBindBuffer(GL_ARRAY_BUFFER, ID);
}

void ShaderAPI_GL::VertexBuffer::Unbind() const {
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}

void ShaderAPI_GL::VertexBuffer::SetData(const void* data, size_t size) const {
    glBindBuffer(GL_ARRAY_BUFFER, ID);
    glBufferData(GL_ARRAY_BUFFER, size, data, GL_STATIC_DRAW);
}

// === VertexArray methods ===
ShaderAPI_GL::VertexArray::VertexArray() {
...
void ShaderAPI_GL::VertexArray::Bind() const {
    glBindVertexArray(ID);
}

void ShaderAPI_GL::VertexArray::Unbind() const {
    glBindVertexArray(0);
}

void ShaderAPI_GL::VertexArray::AddVertexAttribute(unsigned int index, int size, unsigned int type, bool normalized, size_t stride, const void* pointer) const {
    glVertexAttribPointer(index, size, type, normalized ? GL_TRUE : GL_FALSE, stride, pointer);
    glEnableVertexAttribArray(index);
}

// === ShaderAPI_GL methods ===
